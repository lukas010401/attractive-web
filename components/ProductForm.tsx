...
function friendlyError(error: unknown) {
  ...
  if (!message || message.includes('Microsoft.EntityFrameworkCore') || message.length > 260) {
    return 'Enregistrement impossible. VÃ©rifiez les champs puis rÃ©essayez.';
  }
  ...
  function parseSpecifications() {
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      if (separatorIndex <= 0 || separatorIndex === trimmedLine.length - 1) {
        return { specs: [], error: `La spÃ©cification ligne ${index + 1} doit Ãªtre au format "Nom: valeur".` };
      }
    ...
    <form onSubmit={submit} className="grid gap-6 rounded-[1.5rem] bg-white/75 p-6 shadow-soft">
      <section className="grid gap-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Nom du produit" help="Exemple : SÃ©rum Ã©clat visage">
            <input required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} className={inputClass} />
          </Field>
          <Field label="RÃ©fÃ©rence / SKU" help="Optionnel, utile pour identifier le stock.">
      ...
      <section className="grid gap-4">
        <div>
          <h2 className="font-display text-2xl text-ink">Prix et disponibilitÃ©</h2>
          <p className="mt-1 text-sm text-ink/55">Prix affichÃ© en Ariary et stock disponible Ã  la commande.</p>
        </div>
      ...
      <section className="grid gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/55">La description courte apparaÃ®t dans les cartes produit.</p>
        </div>
        ...
        <Field label="Description dÃ©taillÃ©e">
      ...
      <section className="grid gap-4">
        <div>
          <h2 className="font-display text-2xl text-ink">SpÃ©cifications</h2>
          <p className="mt-1 text-sm text-ink/55">Une spÃ©cification par ligne, au format nom : valeur.</p>
        </div>
        <Field label="Liste des spÃ©cifications" help={'Exemple :\nContenance: 50 ml\nType de peau: Tous types'}>
          <textarea
      ...
      <section className="grid gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/55">Publie le produit quand il doit apparaÃ®tre dans le catalogue.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Statut du produit">
            <select value={form.status} onChange={event => setForm({ ...form, status: event.target.value })} className={inputClass}>
              ...
              <option value="Hidden">MasquÃ©</option>
          ...
          <Field label="Mise en avant">
            <span className="flex min-h-[50px] items-center gap-3 rounded-2xl border border-cocoa/15 bg-white px-4 py-3">
              ...
              Afficher sur lâ€™accueil
            </span>
          </Field>
        </div>
        <Field label="Images produit" help={files.length ? `${files.length} fichier(s) sÃ©lectionnÃ©(s)` : 'JPG, PNG ou WEBP. Plusieurs images possibles.'}>
          <input
            type="file"
            multiple
            accept="image/*"
        ...
        {(product?.images?.length || filePreviews.length) ? (
          <div className="grid gap-4">
            {product?.images?.length ? (
              <div>
                <h3 className="text-sm font-semibold text-ink">Images enregistrÃ©es</h3>
            ...
            {filePreviews.length ? (
              <div>
                <h3 className="text-sm font-semibold text-ink">Nouvelles images sÃ©lectionnÃ©es</h3>
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
                  {filePreviews.map((preview, index) => (
                    <div key={preview} className="overflow-hidden rounded-2xl border border-cocoa/10 bg-sand">
                      <img src={preview} alt={files[index]?.name || 'Image sÃ©lectionnÃ©e'} className="h-32 w-full object-cover" />
